(function (global, factory) {
    ...
    if (len > 2) {
      ...
      if (len === 3 && type !== "object" && type !== "function") {
        ...
        for (var i = 2; i < len; i++) {
          ...
          if (_p.join) arr = _p;
          else (arr = SHARED_TEMP_ARRAY)[0] = _p;
  ...
  function createLinkedState(component, key, eventPath) {
    ...
    return function (e) {
      ...
      if (isString(eventPath)) {
        ...
        v = t.nodeName
          ? (t.nodeName + t.type).match(/^input(check|rad)/i)
            ? t.checked
            : t.value
          : e;
      }
  ...
  function idiff(dom, vnode, context, mountAll) {
    ...
    if (
      ...
      out.firstChild instanceof Text
    ) {
    ...
    if (attrs) {
      for (var _name2 in attrs) {
        if (
          !(_name2 in old) ||
          attrs[_name2] !=
            (_name2 === "value" || _name2 === "selected" || _name2 === "checked"
              ? dom[_name2]
              : old[_name2])
        ) {
    ...
    if (!skip) {
      ...
      if (initialBase && base !== initialBase) {
        if (baseParent && base !== baseParent)
          baseParent.insertBefore(base, nextSibling || null);
        if (
          !toUnmount &&
          initialComponent === component &&
          !initialChildComponent &&
          initialBase.parentNode
        ) {
  ...
  extend(Component.prototype, {
    ...
    linkState: function linkState(key, eventPath) {
      ...
      return (
        c[cacheKey] || (c[cacheKey] = createLinkedState(this, key, eventPath))
      );
    },
